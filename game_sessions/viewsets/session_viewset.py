# src/game_sessions/views/session_viewset.py
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def remove_player(self, request, pk=None):
        ...
        # 2) supprime aussi le snapshot d'argent
        SessionMoney.objects.filter(
            session=session,
            player=player
        ).delete()
        # 3) supprime toutes les "futures" du joueur pour cette session
        Future.objects.filter(
            session=session,
            player=player
        ).delete()
